...
def service(service):
    """
    Adds policies to service with STRICT mode.
    ...
    caching = rawobj.PolicyConfig("caching", {"caching_type": "strict"})
    service.proxy.list().policies.insert(0, batcher, caching)
    return service


def test_caching_batching_strict_mode(prod_client, openshift, application):
    """
    Test caching policy with caching mode set to RESILIENT
        ...
        - tests if:
            ...
            - response with new valid credentials have status_code == 200
