...
def application(application):
    ...
    app_plan.limits(metric).create({"metric_id": metric.entity_id, "period": "week", "value": 10})
...
@pytest.fixture(scope="module")
def backend_listener_internal_api_endpoint(backend_listener_url, service, application, service_token):
    """
    ...
    def _backend_listener_internal_api_endpoint(endpoint):
        endpoint = (
            endpoint.replace("{SERVICE_ID}", str(service.entity_id))
...
def data():
    ...
    return {
        ...
        "stats": [("GET", "/services/{SERVICE_ID}/stats/", "404")],
...
def test_internal_backend_listener(
    data, prometheus_response_codes_for_metric, auth_headers, backend_listener_internal_api_endpoint, prometheus
):
    """
