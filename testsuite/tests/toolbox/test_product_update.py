...
def modify_product(toolbox_copy, service):
    ...
    for status in ["error_status_auth_failed", "error_status_auth_missing", "error_status_no_match"]:
...
def modify_mapping_rules(modify_product, modify_metrics, service, modify_methods):
    ...
    maps.append(proxy.mapping_rules.create(rawobj.Mapping(modify_metrics[0], pattern="/ip", http_method="GET")))

    maps.append(proxy.mapping_rules.create(rawobj.Mapping(modify_metrics[0], pattern="/anything", http_method="POST")))

    maps.append(proxy.mapping_rules.create(rawobj.Mapping(hits, pattern="/delete", http_method="DELETE")))

    maps.append(proxy.mapping_rules.create(rawobj.Mapping(hits, pattern="/put", http_method="PUT")))

    maps.append(proxy.mapping_rules.create(rawobj.Mapping(modify_methods[0], pattern="/patch", http_method="PATCH")))

    maps.append(proxy.mapping_rules.create(rawobj.Mapping(modify_methods[0], pattern="/anything", http_method="HEAD")))

    maps.append(
        proxy.mapping_rules.create(rawobj.Mapping(modify_metrics[0], pattern="/anything", http_method="OPTIONS"))
    )
...
@pytest.fixture(scope="module")
def modify_apps_account(
    modify_product, request, custom_application, custom_user, custom_account, testconfig, my_applications_plans
):
...
def modify_policies(modify_product, service):
    """Add new policy"""
    # pylint: disable=unused-argument
    return service.proxy.list().policies.append(rawobj.PolicyConfig("3scale_batcher", {"batch_report_seconds": 50}))
...
def modify_activedocs(modify_product, custom_active_doc, request, oas3_body, service, my_activedoc):
    ...
    # pylint: disable=too-many-arguments
    rawad = rawobj.ActiveDoc(
        name=blame(request, "activedoc2"), service=service, body=oas3_body, description=blame_desc(request)
    )
...
def test_metrics_methods_maps_in_product(
    toolbox_update, modify_product, service, dest_client, dst_product, modify_metrics, my_metrics, modify_mapping_rules
):
    ...
    maps = (
        re.findall(r"created (\d+) mapping rules\n\ntarget service missing", stdout, re.MULTILINE)
        or re.findall(r"copying all service ActiveDocs\n\ncreated (\d+) mapping rules", stdout, re.MULTILINE)
        or re.findall(r"destroying all mapping rules\n\ncreated (\d+) mapping rules", stdout, re.MULTILINE)
        or re.findall(
            r"updated proxy of \d+ to match the original\n\ncreated (\d+) mapping rules", stdout, re.MULTILINE
        )
    )
