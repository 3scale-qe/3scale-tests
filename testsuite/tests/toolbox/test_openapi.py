...
OAS_FILES = {
    "oas2": ["testsuite.resources.oas2", "uber.json"],
...
POLICIES = {
    "policies_config": [
        {"name": "apicast", "version": "builtin", "configuration": {}, "enabled": True},
        {
            ...
            "configuration": {
                "type": "blacklist",
                "scopes": [{"realm_roles": [], "client_roles": [{"name": "read"}, {"name": "write"}]}],
            },
            "enabled": True,
        },
    ]
...
def test_import(import_oas, oas):
    ...
    for path in oas["file"]["paths"].keys():
        for method in oas["file"]["paths"][path].keys():
            path_url = {
                "oas2": lambda: f"{oas['file']['basePath']}",
                "oas3": lambda: f"{urlparse(oas['file']['servers'][0]['url']).path}",
            }[oas["type"]]
            path_url = f"{path_url()}{path}"
            assert re.findall(rf"^Created {method.upper()} {path_url}\$ endpoint$", ret["stdout"], re.MULTILINE)
    if oas["type"] == "oas3":
...
def test_request(import_oas, oas, rhsso_service_info, application):
    ...
    if oas["type"] == "oas3":
        ...
        pol.update(POLICIES)
        application.register_auth("oidc", OIDCClientAuth.partial(rhsso_service_info, location="authorization"))
        path = "/anything/pets"
