...
def test_list1(empty_list, service, create_cmd):
    ...
    to_cmp = fr"{service['id']}\t{service['name']}\t{service['system_name']}"
    assert re.findall(to_cmp, ret['stdout'])
...
def test_list2(empty_list, service, threescale, create_cmd):
    ...
    to_cmp = fr"{out_variables['ser1_entity']['id']}\t{out_variables['ser1_entity']['name']}\t"
    to_cmp += fr"{out_variables['ser1_entity']['system_name']}"
    assert re.findall(to_cmp, ret['stdout'])
    to_cmp = fr"{out_variables['ser2_entity']['id']}\t{out_variables['ser2_entity']['name']}\t"
    to_cmp += fr"{out_variables['ser2_entity']['system_name']}"
    assert re.findall(to_cmp, ret['stdout'])
...
def test_apply1(service, threescale, create_cmd):
    ...
    # pylint: disable=unused-argument
    cmd = fr"{out_variables['ser1_entity']['system_name']} --description='updated' "
    cmd += fr"--name={out_variables['ser1_entity']['name'] + '_updated'} "
    cmd += r'--authentication-mode=oidc --deployment-mode="hosted"'
    ret = toolbox.run_cmd(create_cmd('apply', cmd))
    assert not ret['stderr']
    assert re.findall(fr"Applied Service id: {out_variables['ser1'][1]}", ret['stdout'])
...
def test_check_services_values():
    ...
    toolbox.check_object(out_variables['ser1_entity'], attr_list, [
        '1', 'hosted',
        out_variables['ser1_name'], out_variables['ser1_entity']['system_name']])
    toolbox.check_object(out_variables['ser2_entity'], attr_list, [
        'oidc', 'hosted', 'service2 desc',
        out_variables['ser2_name'], out_variables['ser2_entity']['system_name']])
    toolbox.check_object(out_variables['ser3_entity'], attr_list, [
        'oidc', 'hosted', 'updated',
        out_variables['ser1_name'] + '_updated', out_variables['ser1_entity']['system_name']])
