...
def test_list2(empty_list, service, my_activedoc, threescale, create_cmd):
    ...
    assert re.findall(to_cmp, ret['stdout'])
    to_cmp = fr"{out_variables['ac2_entity']['id']}\t{out_variables['ac2_entity']['name']}\t"
    to_cmp += fr"{out_variables['ac2_entity']['system_name']}\t{service['id']}\ttrue\ttrue"
    assert re.findall(to_cmp, ret['stdout'])
...
def test_apply1(service, threescale, create_cmd):
    ...
    assert not ret['stderr']
    assert re.findall(fr"Applied ActiveDocs id: {out_variables['ac1'][1]}", ret['stdout'])
...
def test_check_active_docs_values():
    ...
    toolbox.check_object(out_variables['ac1_entity'], attr_list, [
        out_variables['ac1_name'], False, False, out_variables['ac1_entity']['system_name']])
    toolbox.check_object(out_variables['ac2_entity'], attr_list, [
        out_variables['ac2_name'], True, True, out_variables['ac2_entity']['system_name']])
    toolbox.check_object(out_variables['ac3_entity'], attr_list, [
