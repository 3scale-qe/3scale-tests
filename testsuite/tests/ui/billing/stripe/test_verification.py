...
@pytest.fixture(
    ...
    autouse=True,
    params=[
        pytest.param(("4000002500003155", True, False), id="EU card, 3ds verification required"),
        pytest.param(("4242424242424242", False, True), id="US card, no verification"),
        pytest.param(("4000002030000002", False, False), id="EU card, no verification"),
    ],
)
def card_setup(request, custom_card):
    """Card setup"""
    custom_card(request.param[0], verify_3ds=request.param[1], postal_required=request.param[2])
