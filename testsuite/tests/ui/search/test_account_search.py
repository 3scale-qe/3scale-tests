...
def test_search_multiple_accounts(login, navigator, custom_account, request):
    ...
    """
    name = blame(request, "")
    params = [dict(org_name=f"{name}_name", username=blame(request, "username"),
                   email=f"{blame(request, 'email')}@anything.invalid", password="123456"),
              dict(org_name=f"{name}_name2", username=blame(request, "username"),
                   email=f"{blame(request, 'email')}@anything.invalid", password="123456"),
              dict(org_name=f"{name}", username=blame(request, "userame"),
                   email=f"{blame(request, 'email')}@anything.invalid", password="123456"),
              dict(org_name=blame(request, "RedHat"), username=blame(request, "username"),
                   email=f"{blame(request, 'email')}@anything.invalid", password="123456")
              ]
    for param in params:
