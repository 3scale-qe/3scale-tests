...
@pytest.mark.parametrize("entity_type,entity", [
...
def test_application_usage(api_client, request, entity_type, entity, granularity, max_allowed_delta, threescale):
    ...
    threescale.analytics._list_by_resource(resource_id=entity_id, resource_type=entity_type,
                                           period=None, since=yesterday.isoformat(),
                                           until=(yesterday + max_allowed_delta).isoformat(), granularity=granularity)
