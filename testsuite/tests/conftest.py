...
def pytest_collection_modifyitems(session, config, items):
    ...
    # rhsso_fixture = ["rhsso_setup"]

    def _is_sandbag(item, markers):
        if "sandbag" in markers or "xfail" in markers:
            return True
    ...
    for item in items:
        ...
        if (
            (config.option.drop_fuzz and "fuzz" in markers)
            or (config.option.drop_nopersistence and "nopersistence" in markers)
            or (config.option.drop_sandbag and _is_sandbag(item, markers))
        ):
            deselected.append(item)
            continue

        if config.option.sandbag_only or config.option.sandbag:
            if _is_sandbag(item, markers):
