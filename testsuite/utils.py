...
def blame(request: 'FixtureRequest', name: str, tail: int = 3) -> str:
    ...
    # When Sphinx search works with strings generated by blame(), parts separated by '-' symbol
    # should be no shorter than 3 characters (default Sphinx indexing settings in Porta).
    # And there are cases when _whoami() returns less than 3 characters.
    whoami = _whoami().replace("-", "")
    if len(whoami) < 3:
        whoami += generate_tail(3)[:3]
